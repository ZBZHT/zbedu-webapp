<template>
  <div class="hnTrainingHead">
    <div>
        <navgation-head></navgation-head>
    </div>
    <div class="hnTraining">
        <el-row>
            <el-col :span="6">
              <el-tree
                node-key="courseId"
  ...
  mounted(){
    ...
    axios.get("/readJson/bannerLeftData",{
      ...
      //console.log(res.data)
          this.data = res.data[5].children;
          var nowid = this.$store.state.noTree.courseId
            if(nowid){
              var courseIdd = nowid
            }else{
              courseIdd = 1001
          ...
          setTimeout(() => {
            this.$refs.vuetree.setCurrentKey(courseIdd)
          }, 20)
      ...
      var checkArrPPT =[]
      var checkArrHomeWork = ''
      checkArrHomeWork = checkArrHomeWork + '信息化教学手段和传统汽车强化训练' + '/'
      checkArrHomeWork = checkArrHomeWork + '汽车检测与维修专业数字化资源开发与信息化教学的实施'
      checkArrPPT.push('信息化教学手段和传统汽车强化训练')
      checkArrPPT.push('汽车检测与维修专业数字化资源开发与信息化教学的实施')
    //评论一系列
      this.user = this.$store.state.username;
      // console.log(this.user)
      this.getComment(checkArrPPT);
      console.log(checkArrPPT)

    //请求PPT
      axios.post("/readResource/getPPT",{
        data:{
          userId:this.userId,
          fileName: checkArrHomeWork
        }
      }).then((res)=>{
      ...
      if(data.children){
        ...
        //请求课件和课后作业给后台发送的数组
          ...
          //请求课后作业
            //拼接地址
          var checkArrHomeWork = ''
          checkArrHomeWork = checkArrHomeWork + node.parent.data.label + '/'
          checkArrHomeWork = checkArrHomeWork + data.label
          console.log(checkArrHomeWork)
          axios.get("/readTestQuestion/getHomeWork",{
            params:{
                userId:this.userId,
          ...
          var checkArrPPT =[]
          checkArrPPT.push(node.parent.data.label)
          checkArrPPT.push(data.label)

          //请求课件
          axios.post("/readResource/getPPT",{
            data:{
              userId:this.userId,
              fileName: checkArrPPT
            }
          }).then((res)=>{
            //console.log(res.data.result.textBookList);
            this.lists = res.data.result.courseList;
...
.hnTraining .teachingBookImg{
    width:90%;
    box-shadow: 3px -3px 15px #000;
    margin-top:35px;
}
.hnTraining #courseWorkPage{
    width:80%;
    margin:0 auto;
    height:700px;
    overflow:auto;
...
.hnTraining #courseppt{
    width:100%;
}
.hnTraining .courseImg{
    height:100%;
    position:relative;
    cursor:pointer;
}
.hnTraining .coursepptImg{
    width:100%;
    height:100%;
}
.hnTraining .pptPrev{
...
.hnTraining .courseDescribe .title{
    font-weight:bolder;
}
.hnTraining .courseDescribe .desc{
    text-indent:2em;
}
.hnTraining .exerP{
    font-size:20px;
...
.hnTraining .devDownload{
    width:96.7%;
    height:53px;
    position:absolute;
    background:rgb(82,86,89);
}
.hnTraining .el-tree-node__expand-icon {
    font-size: 16px;
}
.hnTraining .el-tree-node__label{
...
.hnTraining .el-tree-node:focus>.el-tree-node__content {
    background-color: #9f5355;
    color: white;
}
.hnTraining .el-tree-node__content:hover{
    background-color: #9f5355;
    color: white;
}
.hnTraining .el-tabs--border-card>.el-tabs__header .el-tabs__item:not(.is-disabled):hover {
  color: #212529;
}
.hnTraining .el-tabs--border-card>.el-tabs__header .el-tabs__item {
...
.hnTraining .el-tabs--border-card{
    margin-left:20px;
    border: 0;
    border-top: 4px solid #9f5355;
    box-shadow: none;
}
.hnTraining .el-tabs--border-card>.el-tabs__header .el-tabs__item.is-active{
    background-color: #9f5355;
    color: #ffffff;
}
.hnTraining .el-tree--highlight-current .el-tree-node.is-current>.el-tree-node__content{
    background-color: #9f5355;
    color: white;
}
.hnTraining .el-tabs__item {
  padding:0 60px;
  font-size: 18px;
  line-height: 36px;
}
.hnTraining .el-tabs--border-card>.el-tabs__header{
  border: none;
  background-color: #ffffff;
}
.hnTraining .el-tabs__nav{
    float:none;
}
.hnTraining  .el-tabs--border-card>.el-tabs__content{
    padding-bottom: 100px;
}
.hnTraining .exerEngImg{
    width:595px;
    margin:0 auto;
    margin-bottom:20px;
}
.hnTraining .exerEngImg img{
    width:100%;
}
.hnTraining .content-box{
  width: 100%;
  margin-top: 105px;
  height: 700px;
  position: relative;
...
.hnTraining .el-tree-node{
  background-color: #d2d2d2;
}
.hnTraining .el-tree-node__content{
  border-bottom: 3px solid #ffffff;
}
.el-tree-node__expand-icon{
  color: #000;
}
.hnTraining .homework-box{
  width: 100%;
  height:400px;
  overflow:auto;
}
...
.hnTraining .homework-box .homeworkTitle{
  font-weight:bolder;
  margin-top:20px;
}
.hnTraining .homework-box .homeworkDesc{
  margin-left:20px;
}
.hnTraining .homework-box .homework_Answer{
  margin-left:20px;
  margin-top:10px;
}
.hnTraining .appraise-box{
      width: 90%;
      margin:0 5%;
      position: relative;
      top: 20px;
    }
  .hnTraining .courseTitle{
    font-size: 16px;
    /*font-weight: bold;*/
    /*background: red;*/
    margin-top: 10px;
  }
  .hnTraining .course-box .appraiseTitle{
    width: 500px;
    font-size: 16px;
    font-weight: normal;
    /*background: red;*/
  ...
  .hnTraining .appraise-box .appraiseTitle{
    font-size: 14px;
    text-align: left;
    padding: 6px;
  ...
  .hnTraining .AppraiseNowText{
    margin-top: 0px;
    height: 60px;
    width: 70%;
    font-size: 20px;
  }
  .hnTraining .IwantAppraise{
    width: 90%;
    margin-left: 68px;
  ...
  .hnTraining .showAppraise{
    text-align:left;
    margin-bottom:25px;
    margin-left: 68px;
  }
  .hnTraining .AppraiseMsg{
    display:flex;
  }
  .hnTraining .AppraiseUser{
    font-weight:bolder;
    font-size:15px;
    margin-right:10px;
  }
  .hnTraining .AppraiseTime{
    text-align:right;
    font-size:13px;
    margin-top: 5px;
  }
  .hnTraining .AppraiseText{
    margin-left:50px;
  }
  .hnTraining .replyText{
    margin-top: 10px;
    margin-bottom: 5px;
    height: 35px;
    width: 100%;
  ...
  .hnTraining .showReplyText{
    margin-left:50px;
  }
  .hnTraining .replyEventButtons{
    position:relative;
  }
  .hnTraining .replyEvent{
    position:absolute;
    top:0;
    left:0;
  }
  .hnTraining .replyButtons{
    display:flex;
  }
  .hnTraining .design-box{
    width: 560px;
    height: 400px;
    margin-left: 6%;
    position: relative;
